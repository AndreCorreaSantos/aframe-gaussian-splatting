AFRAME.registerComponent("gaussian_splatting", {
	...
	initGL: async function(numVertexes){
		...
		// load shaders
		const vertexShaderCode = await fetch('shaders/vertex.glsl').then(response => response.text());
		const fragmentShaderCode = await fetch('shaders/fragment.glsl').then(response => response.text());
